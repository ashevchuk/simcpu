...
/**
 * `OTIR`/`OTDR` — see "x=10, z=3: OUTI/OUTD/OTIR/OTDR" in
 * ARCHITECTURE.md. The port transfer, the flags, and the pointer
 * direction are already `OUTI`/`OUTD`'s own machinery, proven mid-flight
 * by `z80cpu-outi.test.ts` / `z80cpu-outd.test.ts`; the one thing
 * genuinely new here is the repeat condition — the same "watch `B`
 * alone" shape `INIR`/`INDR` already establish (see
 * `z80cpu-inir-indr.test.ts`), just reached through this family's own
 * decode. `B` seeded to `2` so the transfer runs exactly twice — the
 ...
 * it.
 */
describe('buildZ80Cpu — x=10, z=3: OTIR/OTDR (repeat mechanism)', () => {
  const ADDR_BITS = 7;
  ...
  const F_B_NONZERO = 0b00000010; // N=1 (0xAB's own bit 7), Z=0
  const F_B_ZERO = 0b01000000; // N=0 (0x11's own bit 7), Z=1 — B reached 0
  ...
  it('OTIR walks HL upward and lands PC back on its own opcode until B reaches 0', () => {
    /**
     * 0: 0x01,0x00,0x02  LD BC,0x0200   BC<-0x0200 (B=2, C=0 — the port address)
     * 3: 0x21,0x10,0x00  LD HL,0x0010   HL<-0x10
     * 6: 0xED,0xB3       OTIR           OUT(C)<-RAM[0x10] (0xAB); HL<-0x11,B<-1; PC<-6 (repeats)
     * 6: 0xED,0xB3       OTIR (again)   OUT(C)<-RAM[0x11] (0x11); HL<-0x12,B<-0; PC<-8 (falls through)
     */
    const program = new Uint8Array(128);
    program.set([0x01, 0x00, 0x02], 0);
    program.set([0x21, 0x10, 0x00], 3);
    program.set([0xed, 0xb3], 6);
    program.set([0xab], 0x10);
    program.set([0x11], 0x11);

    run(program, [
      { b: 2, c: 0, h: 0, l: 0, f: 0, pc: 3 }, // LD BC,0x0200
      { b: 2, c: 0, h: 0, l: 0x10, f: 0, pc: 6 }, // LD HL,0x0010
      { b: 1, c: 0, h: 0, l: 0x11, f: F_B_NONZERO, pc: 6 }, // OTIR pass 1 — repeats
      { b: 0, c: 0, h: 0, l: 0x12, f: F_B_ZERO, pc: 8 }, // OTIR pass 2 — falls through
    ]);
  });

  it('OTDR walks HL downward and lands PC back on its own opcode until B reaches 0', () => {
    /**
     * 0: 0x01,0x00,0x02  LD BC,0x0200   BC<-0x0200 (B=2, C=0 — the port address)
     * 3: 0x21,0x11,0x00  LD HL,0x0011   HL<-0x11
     * 6: 0xED,0xBB       OTDR           OUT(C)<-RAM[0x11] (0xAB); HL<-0x10,B<-1; PC<-6 (repeats)
     * 6: 0xED,0xBB       OTDR (again)   OUT(C)<-RAM[0x10] (0x11); HL<-0x0F,B<-0; PC<-8 (falls through)
     */
    const program = new Uint8Array(128);
    program.set([0x01, 0x00, 0x02], 0);
    program.set([0x21, 0x11, 0x00], 3);
    program.set([0xed, 0xbb], 6);
    program.set([0x11], 0x10);
    program.set([0xab], 0x11);

    run(program, [
      { b: 2, c: 0, h: 0, l: 0, f: 0, pc: 3 }, // LD BC,0x0200
      { b: 2, c: 0, h: 0, l: 0x11, f: 0, pc: 6 }, // LD HL,0x0011
      { b: 1, c: 0, h: 0, l: 0x10, f: F_B_NONZERO, pc: 6 }, // OTDR pass 1 — repeats
      { b: 0, c: 0, h: 0, l: 0x0f, f: F_B_ZERO, pc: 8 }, // OTDR pass 2 — falls through
    ]);
  });
});
