...
stack ops (PUSH/POP, CALL/RET Z/NZ/C/NC, RST, DD PUSH/POP IX, EX (SP),HL)
via multi-ring `gateCatchUp` on PC+SP; soft uses `addrBits` + `gateCallStack`
hooks so CALL/RET/RST match the gate's single-byte return stack.
